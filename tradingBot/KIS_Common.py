        ...
        if area == "US":
            ...
            try:
                ...
                if df is None or len(df) == 0:
                    ...
                    if except_riase == True:
                        print("----Third try----")
                        df = KisKR.GetOhlcvNew(
                            stock_code, "D", limit, adj_ok
                        )  # 새로 대체!
...
def GetOhlcv1(area, stock_code, limit=500, adj_ok="1"):
    ...
    df = fdr.DataReader(
        stock_code, GetFromNowDateStr(area, "BAR", -limit), GetNowDateStr(area, "BAR")
    )
...
def GetOhlcv2(area, stock_code, limit=500, adj_ok="1"):
    ...
    if area == "KR":
        ...
        df = web.DataReader(
            stock_code,
            "naver",
            GetFromNowDateStr(area, "BAR", -limit),
            GetNowDateStr(area, "BAR"),
        )
...
def GetIndexOhlcvPyKrx(index_code, limit=500):
    ...
    df = stock.get_index_ohlcv(
        GetFromNowDateStr("KR", "NONE", -limit), GetNowDateStr("KR", "NONE"), index_code
    )
...
def AutoLimitDoAgain(
    botname, area, stock_code, target_price, do_amt, stock_type="NORMAL"
):
    ...
    if area == "KR":
        ...
        # 사야된다
        if do_amt > 0:
            AutoLimitData["TargetAmt"] = stock_amt + abs(
                AutoLimitData["OrderAmt"]
            )  # 최종 목표 수량 (현재수량 + 주문수량)

            try:

                OrderData = KisKR.MakeBuyLimitOrder(
                    stock_code, abs(do_amt), target_price
                )
                ...
                AutoLimitData["OrderTime"] = (
                    GetNowDateStr(area)
                    + str(time_info.tm_hour)
                    + str(time_info.tm_min)
                    + str(time_info.tm_sec)
                )
        ...
        # 팔아야 된다!
            ...
            try:
                ...
                OrderData = KisKR.MakeSellLimitOrder(
                    stock_code, abs(do_amt), target_price
                )
        ...
        AutoLimitData["DelDate"] = GetFromNowDateStr(
            area, "NONE", 10
        )  # 10일 미래의 날짜 즉 10일 후에는 해당 데이터를 삭제처리할 예정 (어자피 하루만 유효한 주문들이다. 하루 지나고 삭제해도 되지만 참고를 위해 10일간 남겨둔다)
        ...
        AutoLimitData["Id"] = (
            AutoLimitData["NowDist"]
            + botname
            + area
            + str(stock_code)
            + str(AutoLimitData["OrderNum"])
            + str(AutoLimitData["OrderNum2"])
            + str(AutoLimitData["OrderTime"])
            + str(do_amt)
            + str(target_price)
        )
        ...
        if do_amt > 0:
            ...
            try:
                ...
                OrderData = KisUS.MakeBuyLimitOrder(
                    stock_code, abs(do_amt), target_price
                )
                AutoLimitData["OrderNum"] = OrderData["OrderNum"]  # 주문아이디1
                AutoLimitData["OrderNum2"] = OrderData["OrderNum2"]  # 주문아이디2
        ...
        # 팔아야 된다!
            ...
            try:
                OrderData = KisUS.MakeSellLimitOrder(
                    stock_code, abs(do_amt), target_price
                )
                AutoLimitData["OrderNum"] = OrderData["OrderNum"]
                AutoLimitData["OrderNum2"] = OrderData["OrderNum2"]
...
def SaveAutoLimitDoAgainData(AutoLimitData):
    ...
    # 파일 경로입니다.
    auto_order_file_path = (
        "/var/autobot/"
        + AutoLimitData["Area"]
        + "_"
        + AutoLimitData["NowDist"]
        + "_"
        + AutoLimitData["BotName"]
        + "AutoOrderList.json"
    )
                ...
                if AutoLimitData["Id"] == AutoOrderId:
                    ...
                    try:
                        ...
                        for OrderInfo in OrderList:
                            if OrderInfo["OrderNum"] == AutoLimitData[
                                "OrderNum"
...
def cal_cagr(start_value, end_value, start_date, end_date):

    sDate = start_date.replace(" 00:00:00", "")
    eDate = end_date.replace(" 00:00:00", "")

    sDate = datetime.strptime(sDate, "%Y-%m-%d")
    eDate = datetime.strptime(eDate, "%Y-%m-%d")

    date_diff = eDate - sDate
    years_invested = date_diff.days / 365.25

    print(end_value, years_invested)
    cagr = ((end_value / start_value) ** (1 / years_invested) - 1) * 100
    return cagr
