...
async def create_stock(stock: Stock):
    ...
    for existing_stock in stocks_data:
        if existing_stock['code'] == stock.code:
            raise HTTPException(status_code=400, detail=f"종목 코드 '{stock.code}'가 이미 존재합니다")
...
async def get_stock(stock_id: int):
    ...
    raise HTTPException(status_code=404, detail=f"ID {stock_id}인 주식 종목을 찾을 수 없습니다")
        ...
        if stock['id'] == stock_id:
            ...
            for j, other_stock in enumerate(stocks_data):
                if i != j and other_stock['code'] == updated_stock.code:
                    raise HTTPException(status_code=400, detail=f"종목 코드 '{updated_stock.code}'가 이미 존재합니다")
...
async def delete_stock(stock_id: int):
    ...
    for i, stock in enumerate(stocks_data):
        if stock['id'] == stock_id:
            ...
            save_stocks(stocks_data)
            return {"message": f"주식 종목 '{deleted_stock['name']} ({deleted_stock['code']})'이 삭제되었습니다"}
...
async def get_stocks_by_market(market: str):
    ...
    stocks_data = load_stocks()
    market_stocks = [stock for stock in stocks_data if stock['market'].upper() == market.upper()]
